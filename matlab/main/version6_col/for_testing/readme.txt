version6_col for MATLAB Production Server

Before you can use the functions packaged by the compiler, you need to do the following:
. Deploy version6_col.ctf to a server instance
. Develop a client application to access the functions.

1. Deploy version6_col.ctf

1.1 Prerequisites to Deployment
Before you can deploy version6_col.ctf you need a server instance to host it. The server 
...
1.2 version6_col.ctf Deployment
To deploy version6_col.ctf, copy it into the server instance's `auto_deploy` folder. The 
