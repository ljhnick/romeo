main_function for MATLAB Production Server

Before you can use the functions packaged by the compiler, you need to do the following:
. Deploy main_function.ctf to a server instance
. Develop a client application to access the functions.

1. Deploy main_function.ctf

1.1 Prerequisites to Deployment
Before you can deploy main_function.ctf you need a server instance to host it. The server 
instance must be configured to have access to MATLAB Runtime 9.4.
...
1.2 main_function.ctf Deployment
To deploy main_function.ctf, copy it into the server instance's `auto_deploy` folder. The 
