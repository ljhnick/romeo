version5_noncol for MATLAB Production Server

Before you can use the functions packaged by the compiler, you need to do the following:
. Deploy version5_noncol.ctf to a server instance
. Develop a client application to access the functions.

1. Deploy version5_noncol.ctf

1.1 Prerequisites to Deployment
Before you can deploy version5_noncol.ctf you need a server instance to host it. The 
...
1.2 version5_noncol.ctf Deployment
To deploy version5_noncol.ctf, copy it into the server instance's `auto_deploy` folder. 
