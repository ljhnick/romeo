version1_fixed1 for MATLAB Production Server

Before you can use the functions packaged by the compiler, you need to do the following:
. Deploy version1_fixed1.ctf to a server instance
. Develop a client application to access the functions.

1. Deploy version1_fixed1.ctf

1.1 Prerequisites to Deployment
Before you can deploy version1_fixed1.ctf you need a server instance to host it. The 
server instance must be configured to have access to MATLAB Runtime 9.5.
* If you do not have a server instance created see [Server 
  Creation](http://mathworks.com/help/mps/creating_a_server.html).
* If you do not have version 9.5 of the MATLAB Runtime installed download the installer 
  from the MATLAB Runtime 
  page(http://www.mathworks.com/products/compiler/mcr/index.html). 
* If you need to configure the server instance to use the version 9.5 of the MATLAB 
  Runtime see [Configuration File 
  Customization](http://www.mathworks.com/help/mps/customize-the-configuration-file.html).

1.2 version1_fixed1.ctf Deployment
To deploy version1_fixed1.ctf, copy it into the server instance's `auto_deploy` folder. 
