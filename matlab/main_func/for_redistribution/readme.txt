...
. Deploy main_func.ctf to a server instance
. Develop a client application to access the functions.

1. Deploy main_func.ctf

1.1 Prerequisites to Deployment
Before you can deploy main_func.ctf you need a server instance to host it. The server 
instance must be configured to have access to MATLAB Runtime 9.0.
* If you do not have a server instance created see [Server 
  Creation](http://mathworks.com/help/mps/creating_a_server.html).
* If you do not have version 9.0 of the MATLAB Runtime installed download the installer 
  from the MATLAB Runtime 
  page(http://www.mathworks.com/products/compiler/mcr/index.html). 
* If you need to configure the server instance to use the version 9.0 of the MATLAB 
  Runtime see [Configuration File 
  Customization](http://www.mathworks.com/help/mps/customize-the-configuration-file.html).

1.2 main_func.ctf Deployment
To deploy main_func.ctf copy it into the server instance's `auto_deploy` folder. The 
