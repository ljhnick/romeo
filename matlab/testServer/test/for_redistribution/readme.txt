test for MATLAB Production Server

Before you can use the functions packaged by the compiler, you need to do the following:
. Deploy test.ctf to a server instance
. Develop a client application to access the functions.

1. Deploy test.ctf

1.1 Prerequisites to Deployment
Before you can deploy test.ctf you need a server instance to host it. The server instance 
must be configured to have access to MATLAB Runtime 9.4.
...
1.2 test.ctf Deployment
To deploy test.ctf, copy it into the server instance's `auto_deploy` folder. The server 
