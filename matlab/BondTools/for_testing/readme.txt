BondTools for MATLAB Production Server

Before you can use the functions packaged by the compiler, you need to do the following:
. Deploy BondTools.ctf to a server instance
. Develop a client application to access the functions.

1. Deploy BondTools.ctf

1.1 Prerequisites to Deployment
Before you can deploy BondTools.ctf you need a server instance to host it. The server 
...
1.2 BondTools.ctf Deployment
To deploy BondTools.ctf, copy it into the server instance's `auto_deploy` folder. The 
